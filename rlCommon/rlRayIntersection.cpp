...
bool rl::rayIntersection(const Ray& ray, const BoundingBox& bbox) {
	...
	float t3 = (bbox.minC.x - ray.origin().y) * ray.directionInv().y;
	float t4 = (bbox.maxC.x - ray.origin().y) * ray.directionInv().y;
	float t5 = (bbox.minC.x - ray.origin().z) * ray.directionInv().z;
	float t6 = (bbox.maxC.x - ray.origin().z) * ray.directionInv().z;
